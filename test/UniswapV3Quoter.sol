...
contract UniswapV3QuoterTest is Test, TestUtils {
    ...
    function setUp() public {
        ...
        pool = new UniswapV3Pool(address(token0), address(token1), 5602277097478614198912276234240, 85176);
    ...
    function testQuoteUSDCforETH() public {
        (uint256 amountOut, uint160 sqrtPriceX96After, int24 tickAfter) =
            quoter.quote(UniswapV3Quoter.QuoteParams({pool: address(pool), amountIn: 0.01337 ether, zeroForOne: true}));

        assertEq(amountOut, 66.808388890199406685 ether, "invalid amountOut");
        assertEq(sqrtPriceX96After, 5598789932670288701514545755210, "invalid sqrtPriceX96After");
    ...
    function testQuoteETHforUSDC() public {
        ...
        assertEq(sqrtPriceX96After, 5604469350942327889444743441197, "invalid sqrtPriceX96After");
