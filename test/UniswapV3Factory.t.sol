...
contract UniswapV3FactoryTest is Test, TestUtils {
    ...
    function testCreatePool() public {
        address poolAddress = factory.createPool(address(weth), address(usdc), 10);

        IUniswapV3Pool pool = IUniswapV3Pool(poolAddress);

        assertEq(factory.pools(address(usdc), address(weth), 10), poolAddress, "invalid pool address in the registry");
